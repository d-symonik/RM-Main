...
function animateCount(el, target) {
	...
	const render = (time) => {
		...
		el.textContent = Math.round(target * (1 - (1 - progress) ** 3)).toLocaleString();
...
function showFallback(message) {
	...
	p.className = 'allocation-status allocation-status--error';
	p.textContent = message;
	progressList.replaceChildren(p);
...
async function loadProgress() {
	...
	try {
		...
		console.error('Unable to load protocol progress.', error);
